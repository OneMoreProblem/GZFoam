...
\Author{Konstantin Barkalov $^{1}$\orcidA{}, Ilya Lebedev $^{1}$\orcidB{}, Marina Usova$^{1}$\orcidC{}, Daria Romanova$^2$\orcidD{}, Sergei Strijhak$^2$\orcidE{} and Daniil Ryazanov$^2$\orcidF{}}

% MDPI internal command: Authors, for metadata in PDF
\AuthorNames{Konstantin Barkalov, Ilya Lebedev, Marina Usova, Daria Romanova, Sergei Strijhak and Daniil Ryazanov}
...
The comparison of the results of calculating the velocity profile with the experimental data of the Research Institute of Mechanics of Moscow State University in different sections with a change in the angle of inclination of the flume is carried out. The search for the optimal coefficients of the turbulence model is performed by minimizing the objective function of the divergence of the velocity profile in the flume. % - RMSE.

The search for the global minimum of the objective function was performed using the global search agorithm implemented in Globalizer software \cite{globalizerSystem}. In addition, a fully connected neural network with one hidden layer was used to approximate the value of the objective function by the values of the constants of the turbulent model. The MLPRegressor class from the Scikit-learn library was used to build the objective function approximation . 

This paper took an interdisciplinary approach that helped find the optimal value of six turbulence model parameters using the OpenFOAM open platform and Globalizer. The minimum value for the objective function was obtained for the case of a tray angle of 33 degrees. The one calculation of the objective function for the given parameter values took on average 15 minutes using 8 MPI processes on the node of  the Lobachevsky University supercomputer, and the total computation time was 24 hours.  
