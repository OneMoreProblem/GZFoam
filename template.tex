...
\Title{Turbulence Model Optimization for Slope Flow with AI}

% MDPI internal command: Title for citation in the left column
\TitleCitation{Turbulence Model Optimization for Slope Flow with AI}
...
\newcommand{\orcidauthorD}{0000-0002-0722-6884} % Add \orcidD{} behind the author's name
\newcommand{\orcidauthorE}{0000-0002-0722-6884} % Add \orcidF{} behind the author's name
\newcommand{\orcidauthorF}{0000-0002-0722-6884} % Add \orcidG{} behind the author's name

% Authors, for the paper (add full first names)
\Author{Konstantin Barkalov $^{1}$\orcidA{} and Ilya Lebedev $^{1}$\orcidB{} and Marina Usova$^{1}$\orcidC{} and Daria Romanova\orcidD{} and Sergei Strijhak\orcidE{} and Daniil Ryazanov\orcidF{}}

% MDPI internal command: Authors, for metadata in PDF
\AuthorNames{Konstantin Barkalov and Ilya Lebedev and Marina Usova and Daria Romanova and Sergei Strijhak and Daniil Ryazanov}

% MDPI internal command: Authors, for citation in the left column
\AuthorCitation{Barkalov, K.; Lebedev, I., Usova, M.; Romanova D.; Strijhak S.; Ryazanov D.}
...
\abstract{При помощи глобальной оптимизации подбираются коэффициенты модели турбулентности для склоновых потоков. Склоновый поток моделируется при помощи метода конечного объема и $k-\omega\ SST$-модели турбулентности. Выбор оптимальных значений коэффициентов модели турбулентности осуществлялся с использованием global search algorithm. Для ускорения процедуры оптимизации целевая функция аппроксимировалась при помощи нейронной сети. Были получены оптимальные для склоновых потоков коэфициенты турбулентности, проведено сравнение расчетных данных с эксперементальными.
}
...
Изменению климата сопуствуют опасные природныя явления, которые в последнее время наблюдаются все чаще, что неизбежно влияет на жизнь человека и его повседневную деятельность. Изучение этих вопросов состоит в построении математических моделей, анализе геофизических данных, данных численных и натурных эксперементов. Современный тренд анализа геоданных связан с использованием методов машинного обучения и искусственных нейронных сетей.

Оползни, сели и лавины относятся к опасным геологическим явлениям. Они являются склоновыми процессами, связанными с отрывом горных пород, перемещением их по склону под влиянием силы тяжести и приводящим к необратимым преобразованиям в рельефе. Изучение схода оползней, селей и лавин, их предсказания и обнаружения является актуальной задачей в связи с влиянием этих опасных явлений на жизнь людей и городскую инфраструктуру.

Оползни различают по основным причинам возникновения. Они бывают абразионные, эрозионные, техногенные и естественно-техногенные. По механизму смещения различают соответственно оползни блоковые, сдвига, растяжения, разжижения \cite{Pendin2015}.

The data show that in total $55\;997$ people were killed in $4862$ distinct landslide events covering the period from January 2004 to December 2016. The spatial distribution of landslides is heterogeneous, with Asia representing the dominant geographical area \cite{Froude2018}.
...
Большое количество оползней, обвалов и селей происходит на территории Европейских стран (Швейцария, Австрия, Италия, Франция, Исландия). Подобные опасные явления происходят в России, например в Краснодарском крае РФ и на Северном Кавказе, в связи с обильными осадками в этих регионах, с наличием горного рельефа и в связи с застройкой территории (линейные и площадные объекты) в последние годы \cite{hungr2005landslide}. 2021 год является аномальным по количеству выпавших осадков на обозначенных выше территориях России (Сочи, Красная Поляна), что приводит к разливу горзных рек и повышению риска схода склоновых потоков. Отмечены катастрофические сходы селевых масс, приведшие к повреждению дорожной техники и перекрытию дорожного полотна \cite{Harch2020}.

Наиболее известная задача с оползнем связана с оценкой устойчивости оползневого склона для статических условий и сейсмического воздействия. Моделирование же схода склонового потока позволяет предсказать разрушения вызванные этим явлением, грамотно расположить защитные сооружения и жизненно-важные объекты.

Такие задачи решаеются с помощью конечно-разностных методов \cite{Bernander2016}, метода контрольного объема \cite{liu2007application}, дискретных элементов \cite{Liu2020} клеточных автоматов \cite{piegari2006cellular} и гибридных методов. Сели можно моделировать с помощью двухжидкостной модели в основе которой используется модель объема в жидкости – Volume of Fluid \cite{Hirt1981}. Перечисленные методы реализованы в различных коммерческих и открытых программных комплексах: MN2D \cite{Naaim2002}, TITAN2D \cite{Pitman2003}, RAMMS.

Ранее проводилось изучения снежных лавин с использованием методов вычислительной гидродинамики для неньютоновских сред  и данных лабораторного эксперимента, в том числе с использованием исторических данных по лавинам на территории Японии \cite{Oda2011, Yamaguchi2017}. Аналогичные работы проводились по снежным лавинам на территории Швейцарии, Австрии и Италии. В Университете Исландии проводилась серия работ по изучению снежных лавин с помощью лабораторного эксперимента в специальных лотках и измерительного оборудования для изучения лавин в Исландии  \cite{IceThesKatr, IceThesJon}. В работе \cite{Jaedicke2006} представлен эксперимент со спуском водоснежного потока в Давосе, Швейцария. Проводились измерения глубина потока для лавины из сухого снега и для водоснежного потока.

С помощью наивного байесовского классификатора??? уже выполнялась калибровка коэффициентов в $\kappa-\epsilon$ модели турбулентности при изучении процесса распространения примесей в городской застройке \cite{Guillas2014}.

Для моделирования и предсказания склоновых потоков используется широкий круг анализа климатических, геологических и гидрологических данных. В свете последних тенденций огромные потоки данных получаемых со спутников из эксперементов и математического моделирования обрабатываются при помощи машинного обучения, что позволяет разработать эффективные модели этих процессов \cite{GeoML, Ma2020}.

Методы, основанные на сверточных нейронных сетях (CNN), способны извлекать устойчивые пространственные и спектральные особенности \cite{Maggiori2017}. Комбинация спутникового изображения и топографических данных может быть использована для обобщения извлеченных особенностей, чтобы идентифицировать склоновые потоки на спутниковых снимках \cite{Qin2021, Prakash2021}.

В модель склоновых потоков, как правило, входят несколько параметров, значения которых нельзя указать заранее, но можно выбрать, исходя из соответствия результатов численного расчета и имеющихся экспериментальных данных. Подобная задача является задачей глобальной оптимизации с целевой функцией вида черный ящик, т.к. конкретный вид целевой функции не известен, есть только алгоритм вычисления ее значений.

Сложность исследуемых явлений и процессов отражается в сложности соответствующих математических моделей и численных методов их анализа. В настоящее время основным (и зачастую единственно возможным) инструментом подобного анализа является суперкомпьютерное моделирование поведения объекта, для чего широко используюется открытое программное оеспечение. Известным примером открытого программного обеспечения подобного рода является OpenFOAM -- открытая платформа для численного моделирования задач механики сплошных сред \cite{OpenFOAM}. 
...
Для изучения параметров склоновых потоков целесообразно использовать междисциплинарные подходы из областей вычислительной и эксперементальной гидродинамики, теории оптимизации, параллельных вычислений и нейронных сетей. В данной работе используются результаты эксперемента в гидролотке с различным углом наклона, математическая модель для моделирования двухфазного потока, методы глобальной оптимизации и искусственная нейронная сеть. В основе математической модели используется осредненное уравнение Рейнольдса и двухпараметрическая модель турбулентности. 
...
%ННГУ
\subsection{OPTIMIZATION OBJECT FUNCTION}
...
The simulations were conducted using a computer cluster at Lobachevsky University of Nizhni Novgorod (operated under CentOS 7.2 operation system). Each cluster node included two  Intel Sandy Bridge E5-2660 2.2 GHz processors, 64 Gb RAM; the central processor unit had 8 cores. 
...
В работе проведен расчет двухфазного течения в лотке с использованием решателя interFoam, математической модели URANS и k-omega SST модели турбулентности. В процессе оптимизации исследовались попарно шесть констант, которые вносят наибольший вклад в значение турбулентной вязкости.

Выполнено сравнение результатов расчета профиля скорости с данными эксперимента НИИ Механики МГУ в различных сечения при изменении угла наклона лотка. Выполнен поиск оптимальных коэфициентов модели турбулентности путем минимизации целевая функция расхождения профиля скорости в лотке - RMSE.

MLPRegressor класс из Scikit-learn библиотеки был использован для построения  целевой функции аппроксимации. Выполнена оптимизация целевой функции с использованием критерия Липшица и ПО Globalizer Software, разработанного в ННГУ им. Н.И. Лобачевского. При этом дополнительно была использована полносвязанная нейронная сеть с одним скрытым слоем для аппроксимации значения целевой функции по значениям констант турбулентной модели.

В данной работе использовался междисциплинарный подход, который помог найти оптимальное значение шести с использованием открытой платформы OpenFOAM и ПО Globalizer. Минимальное значение для целевой функции получилось для случая угла лотка в 33 градусов. Один расчет целевой функции при заданных значениях параметров занимал в  среднем 15 минут с использованием 8 MPI-процессов на узле вычислительного кластера <<ЦКП Лобачевский>>.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\authorcontributions{``Conceptualization, K.B. and D.R.; methodology, Rom.D. and K.B.; software, I.L., Daniil Ryazanov, Marina Usova and  Romanova Daria; validation,  Romanova Daria and Ilya Lebedev; formal analysis, S.S., M.U.; investigation, S.S.; resources, I.L.; data curation, Rom.D. and M.U.; writing---original draft preparation, K.B., S.S. and Rom.D.; writing---review and editing, R.D.; visualization, I.L.; supervision, S.S.; project administration, K.B.; funding acquisition, K.B. All authors have read and agreed to the published version of the manuscript.'', please turn to the  \href{http://img.mdpi.org/data/contributor-role-instruction.pdf}{CRediT taxonomy} for the term explanation. Authorship must be limited to those who have contributed substantially to the work~reported.}
