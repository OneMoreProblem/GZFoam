...
Оползни, сели и лавины относятся к опасным геологическим процессам. Они являются склоновыми процессами, связанными с отрывом горных пород, перемещением их по склону под влиянием силы тяжести и приводящим к необратимым преобразованиям в рельефе. Оползни различают по основным причинам возникновения. Они бывают абразионные, эрозионные, техногенные и естественно-техногенные. По механизму смещения различают соответственно оползни блоковые, сдвига, растяжения, разжижения.

Проблема изучения схода оползней, селей и лавин, их предсказания и обнаружения является актуальной задачей в связи с влиянием этого опасного явления на жизнь людей и городскую инфраструктуру.
...
Вычислительные эксперименты проводились на вычислительном кластере ННГУ им. Н.И. Лобачевского (работает под управлением операционной системы CentOS 7.2). Узел кластера располагает 2-я процессорами Intel Sandy Bridge E5-2660 2.2 GHz, 64 Gb RAM. Центральный процессор является 8-х ядерным. Рассмотренные в данной работе методы глобальной оптимизации были реализованы на языке C++, использовался компилятор GCC 5.5.0 и openMPI 4.1.1, для построения аппроксимации целевой функции с помощью нейросети использовалась библиотека машинного обучения Scikit-learn из Python 3.9. Для численного решения задачи, описанной в Section \ref{math_model}, использовалась программная система OpenFOAM 2012.
...
Вначале была выбрана пара наиболее важных параметров $\beta^*$ и $a_1$. 
Для исследования возникающей задачи оптимизации мы применяли оба возможных подхода к ее решению: без использования аппроксимации целевой функции и с использованием аппроксимации.

В первом эксперименте использовался global search alogithm, описанный в subsection \ref{GSA}, без использования аппроксимации. Были заданы параметры методы  $r = 3$ и $\epsilon = 10^{-3}$. За 24 часа было выполнено 100 итераций алгоритма, требуемой точности достичь не удалось. 

Во втором эксперименте для решения этой же задачи использовался подход, описанный в subsection \ref{GSA_Appr}. Вначале было выполнено $K_1 = 30$ итераций алгоритма глобального поиска, после чего начал работать алгоритм с использованием аппроксимации нейросетью. Всего было выполнено $K_1 + K_2 = 65$ итераций алгоритма, после чего произошла остановка по точности. В результате было найдено лучшее значение целевой функции 0.375174. При этом общее время поиска решения сократилось до 16 часов, что обеспечило получение более точного решения задачи за приемлемое время.

На рисунке ~\ref{NN_100_point} приведены точки испытаний и график аппроксимирующей функции, построенной по ним с использованием нейросети (варьировались параметры $\beta^*$ и $a_1$). Наглядно видно наличие нескольких локальных минимумов.

\begin{figure}[H]
\begin{center}
\includegraphics[width=0.8\linewidth]{NN_100_point_.png}
\caption{Значения целевоф функции  и ее аппроксимация, построенная с помощью нейросети (варьировались параметры $\beta^*$ и $a_1$)}
\label{NN_100_point}
\end{center}
\end{figure}

Найденные лучшие значения параметров $\beta^*$ и $a_1$ были зафиксированы, и затем проводилась оптимизация по параметрам $\alpha_{k1}, \alpha_{\omega1}$ и $\alpha_{k2}, \alpha_{\omega2}$. Оптимизация по этим параметрам не дала значительного улучшения целевой функции, было получено значение 0.364464.
