...
$^{2}$ \quad Institute For System Programming of Russian Academy of Science, 109004, Moscow, Russia; Ryazanov Daniil ryazanov@ispras.ru (R.D.), Romanova Daria romanova.d@ispras.ru (Rom.D.)\\
...
\abstract{The paper considers the slope flow simulation and the problem of finding the optimal parameter values of this mathematical model. The slope flow is modeled using the finite volume method, the averaged Reynolds equations model and the $k-\omega\ SST$ turbulence model. The Root Mean Square Error of the calculated flow velocity profile is considered as the objective function in the optimization problem. The analytical form of this function is not known and the calculation of its values involves time-consuming numerical simulation. The optimal values of the coefficients of the turbulence model were found using the global search algorithm. 
\sout{To speed up the optimization procedure, the objective function was approximated using an artificial neural network.} 
The \sout{optimal} coefficients of the turbulence model for slope flows were obtained with virtual sensors, and the calculated data were compared with the experimental ones.
\textcolor{red}{The calibration of turbulence models coefficients for calculating the free surface flows on slopes using multiphase model for interphase tracking has not been performed earlier.} 
\textcolor{green}{To solve the multi-extremal optimization problem arising from the search for the minimum of the loss function for the flow velocity profile we use a new optimization approach using a Peano curve to reduce the dimensionality of the problem.
