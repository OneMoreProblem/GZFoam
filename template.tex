...
Here $N$ is the number of measuring points for horizontal component of velocity over flow depth at the outlet plane. $v_{EXP}^i$ -- horizontal component of velocity at in-the control point obtained by experiment. $v_{k-\omega\ SST}^i$  -- horizontal component of velocity at in-the control point calculated by computational fluid dynamic (CFD).
...
In this work, a two-phase flow in a flume was calculated using the interFoam solver, the URANS mathematical model and the k-omega SST turbulence model. In the optimization process, six constants were investigated in pairs, which make the greatest contribution to the value of turbulent viscosity.

The comparison of the results of calculating the velocity profile with the experimental data of the Research Institute of Mechanics of Moscow State University in different sections with a change in the angle of inclination of the flume is carried out. The search for the optimal coefficients of the turbulence model is performed by minimizing the target function of the divergence of the velocity profile in the flume - RMSE.

The MLPRegressor class from the Scikit-learn library was used to build an objective approximation function. The optimization of the objective function was performed using the Lipschitz criterion and the Globalizer Software developed at the Lobachevsky University. In addition, a fully connected neural network with one hidden layer was used to approximate the value of the objective function by the values of the constants of the turbulent model.

This paper took an interdisciplinary approach that helped find the optimal value of six using the OpenFOAM open platform and Globalizer software. The minimum value for the objective function was obtained for the case of a tray angle of 33 degrees. The one case calculation of the objective function for the given parameter values took on average 15 minutes using 8 MPI processes on the node of supercomputer of the Lobachevsky Center for Collective Use.
