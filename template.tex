...
\Author{Konstantin Barkalov $^{1}$\orcidA{} and Ilya Lebedev $^{1}$\orcidB{} and Marina Usova$^{1}$\orcidC{} and Daria Romanova$^2$\orcidD{} and Sergei Strijhak$^2$\orcidE{} and Daniil Ryazanov$^2$\orcidF{}}
...
\abstract{Using the global optimization method, the coefficients of the turbulence model for slope flows are selected. The slope flow is modeled using the finite volume method and the k - ω SST turbulence model. The choice of the optimal values of the coefficients of the turbulence model was carried out using the global search algorithm. To speed up the optimization procedure, the objective function was approximated using artificial neural network. The optimal coefficients of the turbulence model for slope flows were obtained, and the calculated data were compared with the experimental ones.}
...
The climate change is accompanied by dangerous natural phenomena, which have recently been observed more and more often, which inevitably affects human life and daily activities. The study of these issues consists in the construction of mathematical models, analysis of geophysical data, data from numerical and field experiments. The current trend in the analysis of geodata is associated with the use of machine learning algorithms and artificial neural networks.

The landslides, mudflows and avalanches are classified as dangerous geological phenomena. They are slope processes associated with the separation of rocks, their movement along the slope under the influence of gravity and leading to irreversible changes in the relief. The study of the descent of landslides, mudflows and avalanches, their prediction and detection is an urgent task in connection with the impact of these dangerous phenomena on human life and urban infrastructure. The landslides are distinguished for their main causes. They are abrasive, erosional, man-made and natural-man-made.

The landslides are distinguished for their main causes. They are abrasive, erosional, man-made and natural-man-made. According to the mechanism of displacement, block landslides, shears, stretching, liquefaction are distinguished, respectively \cite{Pendin2015}.
...
A large number of landslides, avalanches and mudflows occur in European countries (Switzerland, Austria, Italy, France, Iceland). Similar dangerous phenomena occur in Russia, for example, in the Krasnodar Territory of the Russian Federation and in the North Caucasus, due to heavy rainfall in these regions, with the presence of mountainous terrain and in connection with the development of the territory (linear and areal objects) in recent years \cite{hungr2005landslide}. The year 2021 is anomalous in terms of the amount of precipitation in the above territories of Russia (Sochi, Krasnaya Polyana), which leads to the flooding of mountain rivers and an increased risk of slope flows. Catastrophic descent of mudflows was noted, which led to damage to road equipment and overlap of the roadway \cite{Harch2020}.

The mudflows are one of the most complex exogenous geological processes that integrate the actions of other geological processes. The mudflow is a complex heterogeneous structure consisting of liquid and solid components. The solid element consists of mineral particles that are non-uniform in the granulometric relation.
...
Such problems are solved using finite-difference methods \cite{Bernander2016}, finite volume method \cite{liu2007application}, discrete
elements \cite{Liu2020} of cellular automata \cite{piegari2006cellular} and hybrid methods. The mudflows can be modeled using a two-fluid model based on the Volume of Fluid model \cite{Hirt1981}. The listed methods are implemented in various commercial and open source software packages: MN2D \cite{Naaim2002}, TITAN2D \cite{Pitman2003}, RAMMS.

Previously, the study of avalanches was carried out using the methods of computational fluid dynamics for non- Newtonian fluids and laboratory experiment data, including the use of historical data on avalanches in Japan \cite{Oda2011, Yamaguchi2017}. Similar work was carried out on avalanches in Switzerland, Austria and Italy. The University of Iceland carried out a series of works on the study of avalanches using a laboratory experiment in special trays and measuring equipment for studying avalanches in Iceland  \cite{IceThesKatr, IceThesJon}. The paper \cite{Jaedicke2006} presents an experiment with the descent of a snow-water stream in Davos, Switzerland. Measurements were made for the flow depth for a dry snow avalanche and for a snow-water flow.

Among similar work, the coefficients were already calibrated in the $\kappa-\epsilon$ turbulence model when studying the process of the propagation of impurities in urban development \cite{Guillas2014}.

A wide range of analysis of climatic, geological and hydrological data is used to model and predict slope flows. In the light of recent trends, huge streams of data received from satellites from experiments and mathematical modeling are processed using machine learning, which makes it possible to develop effective models of these processes \cite{GeoML, Ma2020}.

Methods based on convolutional neural networks (CNN) are able to extract stable spatial and spectral features \cite{Maggiori2017}. The combination of satellite image and topographic data can be used to generalize the extracted features in order to identify slope flows in satellite images \cite{Qin2021, Prakash2021}.

The slope flow model, as a rule, includes several parameters, the values of which cannot be specified in advance, but can be selected based on the correspondence of the numerical calculation results and the available experimental data. Such a task is a global optimization task with a black box objective function, because the specific type of the objective function is not known, there is only an algorithm for calculating its values.

The complexity of the phenomena and processes under study is reflected in the complexity of the corresponding mathematical models and numerical methods of their analysis. Currently, the main (and often the only possible) tool for such analysis is supercomputer modeling of object behavior, for which open source software is widely used. A well-known example of open source software of this kind is OpenFOAM – an open platform for numerical modeling of continuum mechanics problems \cite{OpenFOAM}. 
...
Experimental installations are used to study the parameters of slope flows, such as discharge and velocity profile in various sections.
