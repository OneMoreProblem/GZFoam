...
Вычислительные эксперименты проводились на одном из узлов вычислительного кластера ННГУ им. Н.И. Лобачевского (под управлением операционной системы CentOS 7.2). Узел кластера располагает 2-я процессорами Intel Sandy Bridge E5-2660 2.2 GHz, 64 Gb RAM. Центральный процессор является 8-х ядерным. Для построения системы Globalizer \cite{globalizerSystem} использовался компилятор GCC 5.5.0 и openMPI 4.1.1. Для решения задачи использовался OpenFOAM 2012.

Во время проведения экспериментов были проведены два запуска решения задачи. В первом запуске производилось решение задачи классическим алгоритмом глобального поиска, параметр метода $r = 3$, было выполнено 100 итераций алгоритма. Во втором запуске после 30 итераций алгоритма глобального поиска начинал работать алгоритм с использованием аппроксимации нейросетью, параметр метода $r = 3$, было сделано 65 итераций.

На рисунке ~\ref{NN_100_point} приведено изображение функции, полученной с помощью аппроксимации исходной задачи нейросетью, обученной на 100 точках испытания. Как можно видеть функция имеет несколько локальных минимумов.
