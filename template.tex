...
\subsection{The Use of Approximations in Solving the Optimization Problem}
...
Вычислительные эксперименты проводились на одном из узлов вычислительного кластера ННГУ им. Н.И. Лобачевского (под управлением операционной системы CentOS 7.2). Узел кластера располагает 2-я процессорами Intel Sandy Bridge E5-2660 2.2 GHz, 64 Gb RAM. Центральный процессор является 8-х ядерным. Рассмотренные в данной работе методы глобальной оптимизации были реализованы на языке с++, компилятор использовался компилятор GCC 5.5.0 и openMPI 4.1.1, для построения нейросети использовалась библиотека машинного обучения Scikit-learn на python 3.9 

Для численного решения задачи, описанной в разделе 5, использовалась программная система OpenFOAM 2012.
Оптимизационная задача имеет 12 параметров, физическое значение этих параметров описаны в разделе 5. Ниже перечислены параметры и начальные значения.
\begin{equation}
	\label{kOmegaSstConstantsInit}
	\begin{aligned}
		\gamma_1 = 5 / 9,\ \ \ \beta_1 = 3 / 40,\ \ \ \alpha_{k1} = 0.85,\ \ \ \alpha_{\omega1} = 0.5,\\
...
При решении задачи оптимизации, один расчет целевой функции занимал в среднем 15 минут и производился параллельно на 8 mpi процессах. Решение исходной задачи производилось последовательно, парами по 2 параметра, при этом остальные параметры фиксировались. 

В начале была выбрана пара наиболее важных параметров: $beta^*$ и $a_1$. Для решения возникающей задачи оптимизации использовался АГП описанный в разделе 3,3 с параметрами $r = 3$ и $\epsilon = 0.001$. в силу ограничений по времени было выполнено 100 итераций алгоритма и произошла остановка. Далее для решение этой задачи использовался подход, описанный в разделе 3.5, в начале было выполнено 30 итераций алгоритма глобального поиска после чего начал работать алгоритм с использованием аппроксимации нейросетью, параметр метода $r = 3$ и $\epsilon = 0.001$, было сделано 65 итераций алгоритма, произошла остановка по точности. В результате значение целевой функции было уменьшено до значения 0.375174. 

На рисунке ~\ref{NN_100_point} приведено изображение функции, полученной с помощью аппроксимации задачи нейросетью, обученной на 100 точках испытания (варьировались параметры $beta^*$ и $a_1$). Как можно видеть функция имеет несколько локальных минимумов.

\begin{figure}[H]
\begin{center}
\includegraphics[width=1.0\linewidth]{NN_100_point_.png}
\caption{Изображение функции, полученной с помощью аппроксимации задачи нейросетью, обученной на 100 точках испытания (варьировались параметры $beta^*$ и $a_1$)}
\label{NN_100_point}
\end{center}
\end{figure}

Второй парой оптимизируемых параметров были выбраны $\alpha_{k1} $ и $\alpha_{\omega1} $. Третьей парой были выбраны параметры $\alpha_{k2} $ и $\alpha_{\omega2} $. Оптимизация по этим параметрам не дала значительного улучшения целевой функции, было получено значение 0.364464.
