...
%ННГУ
\subsection{The global search algorithm}
...
%existence of inequality constraints in the problem and the information about the objective function derivative are given in \cite{RefBarkalov,RefGergel1996,RefGergel1997}
...
В проведенном исследовании мы применяли простой способ использования аппроксимации целевой функции в задачах оптимизации.
\begin{enumerate}
	\item используя накопленную поисковую информацию построить аппроксимацию $p(y)$;
	\item найти ee минимум $\overline{y} = \arg \min_{y \in D} p(y)$;
	\item провести очередное испытание в найденной точке $\overline{y}$;
	\item добавить результаты испытания к поисковой информации, и повторить процесс либо до исчерпания вычислительного ресурса, либо до достижения сходимости.
\end{enumerate}
...
На первом этапе проводится $k = K_1$ испытаний с использованием core global search algorithm из section .
Во время выполнения первого этапа происходит накопление поисковой информации, необходимой для построения аппроксимации функции.
...
Алгоритм прекращает свою работу, если выполнено $k>K_{max}$, или же если выполнено условие $\left\|y^k - y^{k+1}\right\| \leq \epsilon$.
